...
export default class StickyCard extends ScrollBase {
    constructor(elem) {
        super(elem)
        this.children = this.elem.querySelectorAll('.project--item')
        this.count = this.elem.children.length - 1
    }
    ...
    onScroll(scrollY) {
        if (scrollY > this.start && scrollY < this.end) {
            ...
            const code = Math.round(this.mapRange(percentage, 0, 100, 0, this.count))
            this.setActive(code)
        }
    }
    ...
    setActive(code) {
        this.children.forEach((child, index) => {
            ...
            if (index < code) {
                ...
                const scale = this.mapRange(scroll, 0, this.animationStart, .9, 1)
